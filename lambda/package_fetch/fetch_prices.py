        ...
        if alerts_topic_arn and os.getenv('USERS_TABLE'):
            ...
            for user in users:
                    ...
                    if (condition == '<=' and current_price <= target_price) or \
                       (condition == '>=' and current_price >= target_price):
                        triggered_messages.append(
                            f"- {config.get('retailer')} {config.get('oilType').replace('_', ' ')}: {current_price} THB/L (Condition: {condition} {target_price})"
                        )
                ...
                if triggered_messages:
                    ...
                    message = (
                        'Hello!\n\n'
                        'The following oil prices have reached your target conditions today:\n\n'
