...
resource "aws_lambda_function" "scrape" {
  ...
  # Option A: keep OUTSIDE VPC so no NAT/SG/port-80 issue
  # no vpc_config block

  # Option B: if you want it inside VPC, add the same vpc_config
  # and also allow outbound port 80 in security_groups.tf
